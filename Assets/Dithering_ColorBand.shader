Shader "Unlit/Dithering_ColorBand"
{
    Properties
    {
        ...
        _AtmoDensity ("Atmosphere Density", float) = 1.0
        ...
        _CloudFalloff ("Cloud Falloff", float) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 startCloud = start_point + ray_direction * tCloudEnter;
                col = lerp(col, _AtmoColor, 1.0 - exp(-terrainLevel * _AtmoDensity * _EXTINCTION_MULT));
                if(!intersect || (terrainLevel < tCloudEnter)) return col;
                ...
                for (tCloud = 0; tCloud < 1.0; tCloud += 0.02)
                {
                    ...
                    startCloud += ray_direction * stepSize * (1-density) * _CloudFalloff;
                ...
                col = lerp(col, _AtmoColor, 1.0 - exp(-min(terrainLevel,length(startCloud-start_point)) * _AtmoDensity * _EXTINCTION_MULT));
